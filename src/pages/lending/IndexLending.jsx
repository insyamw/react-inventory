...
export default function Lending() {
    ...
    const deleteLending = (id) => {
        instance.delete(`lending/${id}`)
            .then(res => {
                setLendings(lendings.filter(stuff => stuff.id !== id));
            })
            .catch(err => {
                        ...
                        <table className="min-w-full text-left text-sm font-light">
                            ...
                            <tbody>
                                {lendings.map((lending, id) => (
                                    <tr key={lending.id} className="border-b dark:border-neutral-500 dark:text-white">
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4">
                                            <Link to={'/lending/edit/' + lending.id} className="px-4 py-2 bg-orange-500 rounded-lg mr-2 font-bold text-white">Edit</Link>
                                            <button type="button" onClick={() => deleteLending(lending.id)} className="px-4 py-2 bg-red-500 rounded-lg mr-2 font-bold text-white">Hapus</button>

                                            {/* bikin logic if (jika) */}
                                            {
                                                // jika ADA data restorations nya maka muncul tulisan "Sudah pernah restore"
                                                lending.restorations.length ? (
                                                    'Sudah pernah restore'
                                                ) : (
                                                    // else
                                                    // maka tampil tombol untuk ke halamann create Restoration (pengembalian barang)
                                                    <Link to={'/lending/restoration/' + lending.id} className="px-4 py-2 bg-green-500 rounded-lg mr-2 font-bold text-white">Restore</Link>
                                                )
                                            }
                                        </td>
                                    </tr>
